...
def get_lane_from_ground_truth(gt: GroundTruth, lane_id: int) -> Lane:
    for lane in gt.lane:
        if lane.id.value == lane_id:
            return lane
    raise RuntimeError(f"Missing data for lane {lane_id}")
...
class LaneState:
    def __init__(self, gt: GroundTruth, lane_id: int):
        self.osi_lane = get_lane_from_ground_truth(gt, lane_id)
        left_boundaries = [
            LaneBoundaryState(gt, id)
...
class LaneBoundaryState:
    def __init__(self, gt: GroundTruth, boundary_id: int):
        self.osi_boundary = get_lane_boundary_from_ground_truth(gt, boundary_id)
        self.segments: list[tuple[np.ndarray, np.ndarray]] = []
        line = self.osi_boundary.boudary_line
        for i in range(len(line) - 1):
            self.segments.append((line[i], line[i + 1]))
